                        ...
                        return (
                            <div key={idx} style={{ position: 'relative', aspectRatio: '4/3', borderRadius: 10, overflow: 'hidden', border: '2px solid #d0f0e0' }}>
                                ...
                                <button type="button" onClick={() => removeOne(idx)}
                                    style={{ position: 'absolute', top: 4, right: 4, width: 20, height: 20, borderRadius: '50%', background: 'rgba(239,68,68,0.9)', border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#fff' }}>
                                    <X size={10} />
                                </button>
...
function TaskModal({ task, initialTab = 'view', onClose, onSave }) {
    ...
    const validate = () => {
        const errs = {};
        if (closeImg.length === 0 && existingClose.length === 0) errs.closeImg = 'At least one near photo required';
        if (farImg.length === 0 && existingFar.length === 0) errs.farImg = 'At least one far photo required';
        return errs;
    };
    ...
    const handleSave = async () => {
        ...
        try {
            await onSave({ ...task, status: 'Submitted', actualCompletionDate: task.actualCompletionDate || todayISO, closeImgs: closeImg, farImgs: farImg });
            setSaved(true);
...
useEffect(() => {
        ...
        setAttLoading(true);
        // Collect all task IDs to load — includes merged siblings if present
    ...
    return ReactDOM.createPortal(
        <div onClick={e => e.target === e.currentTarget && onClose()}
            style={{ position: 'fixed', inset: 0, zIndex: 1060, background: 'rgba(15,23,42,0.58)', backdropFilter: 'blur(7px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16, overflowY: 'auto' }}>
            <div style={{ background: '#fff', borderRadius: 22, width: '100%', maxWidth: 620, maxHeight: '92vh', overflowY: 'auto', boxShadow: '0 32px 80px rgba(0,0,0,0.22)', animation: 'modalIn 0.26s cubic-bezier(0.22,1,0.36,1) both', margin: 'auto' }}>
                ...
                <div style={{ display: 'flex', borderBottom: '1.5px solid #f0f0f8', padding: '0 22px' }}>
                    {[{ id: 'view', label: '📋 Details' }, { id: 'update', label: '✏️ Update Task' }].map(t => (
                        <button key={t.id} onClick={() => setTab(t.id)} style={{ padding: '12px 18px', border: 'none', background: 'none', fontFamily: 'Nunito,sans-serif', fontSize: 13, fontWeight: 800, color: tab === t.id ? '#049edf' : '#9090a8', borderBottom: tab === t.id ? '2.5px solid #049edf' : '2.5px solid transparent', cursor: 'pointer', transition: 'all 0.15s', marginBottom: '-1.5px' }}>{t.label}</button>
                    ))}
                </div>
                ...
                {tab === 'view' && (
                    <div style={{ padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
                            ...
                            {/* ── Site address row ── */}
                            {task.siteAddress && (
                                <InfoRow icon={MapPin} label="Site Address" value={task.siteAddress} accent="#16a34a" />
                            )}
                            {task.isMerged && (
                                <InfoRow icon={Layers} label="Merge Type" value={task.mergeAlongFlag === 'H' ? '↔ Horizontal Merge' : '↕ Vertical Merge'} accent="#7c3aed" />
                            )}
                            ...
                            {attLoading ? (
                                ...
                                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                        ...
                                        <ImageUploadZone label="Near Photo" sublabel="(Close-up)" IconComp={ZoomIn} values={closeImg}
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                        ...
                                        <ImageUploadZone label="Far Photo" sublabel="(Wide shot)" IconComp={ZoomOut} values={farImg}
...
/* ══════════════════════════════════════════
   MERGED GROUP ROW  (collapsed card for table view)
══════════════════════════════════════════ */
function MergedGroupRow({ groupTasks, onView, onEdit }) {
    ...
    // Pick the most actionable task: prefer Open > In Progress > first
    const actionTask = groupTasks.find(t => t.status === 'Open')
        ?? groupTasks.find(t => t.status === 'In Progress')
        ?? firstTask;
    const hoardingCodes = groupTasks.map(t => t.hoardingCode).filter(Boolean);
...
<td className="pg-td">
    <div className="pg-action-wrap">
        <button className="pg-btn-edit" title="Update Task" onClick={() => onEdit({
            ...actionTask,
            mergedTaskIDs: groupTasks.map(t => t.jobTaskID),
        })}>
            <Edit3 size={13} />
        </button>
        <button className="pg-btn-view" title="View Details" onClick={() => onView({
            ...actionTask,
            mergedTaskIDs: groupTasks.map(t => t.jobTaskID),
        })}>
            <Eye size={13} />
    ...
    return (
        <div className="pg-card" style={{ border: '1.5px solid rgba(124,58,237,0.3)', background: 'linear-gradient(135deg,rgba(124,58,237,0.03),#fff)' }}>
            ...
            <div className="pg-card__body">
                ...
                {groupTasks.map(t => (
                    <div key={t.jobTaskID} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 0', borderTop: '1px solid #f0f0f8' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                            <span style={{ fontFamily: 'Nunito,sans-serif', fontSize: 11, fontWeight: 800, color: '#049edf' }}>#{t.jobTaskID}</span>
                            <StatusBadge status={t.status} />
                        </div>
                        <div className="pg-action-wrap">
                            <button className="pg-card__btn-edit" onClick={() => onEdit(t)} title="Update"><Edit3 size={13} /></button>
                            <button className="pg-card__btn-view" onClick={() => onView(t)} title="View"><Eye size={13} /></button>
                        </div>
                    </div>
                ))}
            </div>
...
export default function WorkerTasksPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            // Parallel fetches — including hoardings, sites, and merges
            ...
            /* ── Second pass: mark ALL tasks in a merged job consistently ── */
            const jobMergeFlags = new Map();
            myTasks.forEach(t => {
                if (t.isMerged && !jobMergeFlags.has(t.jobRequestID)) {
                    jobMergeFlags.set(t.jobRequestID, t.mergeAlongFlag);
                }
            });

            const finalTasks = myTasks.map(t =>
                jobMergeFlags.has(t.jobRequestID)
                    ? { ...t, isMerged: true, mergeAlongFlag: t.mergeAlongFlag ?? jobMergeFlags.get(t.jobRequestID) }
                    : t
            );
    ...
    const handleSave = async (data) => {
        ...
        for (const file of data.closeImgs ?? []) {
            ...
            fd.append('PhotoFileType', 'Near Photo'); fd.append('Files', file);
        ...
        for (const file of data.farImgs ?? []) {
            ...
            fd.append('JobTaskAttachID', '0'); fd.append('JobTaskID', String(data.jobTaskID));
            fd.append('JobRequestID', String(data.jobRequestID)); fd.append('HoardingID', String(data.hoardingID));
            fd.append('PhotoFileType', 'Far Photo'); fd.append('Files', file);
            fd.append('PhotoFilePath', ''); fd.append('PhotoFilename', file.name);
            fd.append('LastUpdateDttm', nowISO); fd.append('LastUpdatedBy', String(userId));
            await apiService.uploadJobTaskAttachment(fd);
        }
    ...
    // In displayRows useMemo, replace the grouping key logic:
    ...
    const displayRows = useMemo(() => {
        ...
        sorted.forEach(task => {
            if (task.isMerged) {
                // Group by jobRequestID only — merged tasks always share the same job
                const key = `merged__${task.jobRequestID}`;
                if (mergedGroupMap.has(key)) {
                    rows[mergedGroupMap.get(key)].groupTasks.push(task);
                } else {
                    mergedGroupMap.set(key, rows.length);
                    rows.push({ type: 'merged', groupTasks: [task] });
                }
            } else {
    ...
    const COLS = [
        ...
        { key: 'actualCompletionDate', label: 'Comp. Date', w: '12%', tabletHide: true },
        { key: 'submitDTTM', label: 'Submitted', w: '12%', tabletHide: true },
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 14, marginBottom: 20 }}>
                    {[
                        ...
                        <div key={s.label} onClick={() => { setStatusFilter(statusFilter === s.filter ? 'all' : s.filter); setPage(1); }}
                            style={{ background: 'rgba(255,255,255,0.88)', backdropFilter: 'blur(16px)', borderRadius: 16, padding: '16px 18px', border: statusFilter === s.filter ? `2px solid ${s.color}` : `1.5px solid ${s.border}`, boxShadow: '0 2px 14px rgba(100,100,180,0.07)', display: 'flex', alignItems: 'center', gap: 14, cursor: 'pointer', transition: 'all 0.15s' }}>
                ...
                <div className="pg-container">
                    ...
                    <div className="pg-toolbar">
                        <div className="pg-toolbar__inner">
                            ...
                            <div className="pg-search-box">
                                ...
                                <input placeholder="Search by task, job, hoarding, address, status…" value={search}
                    ...
                    <div className="pg-pagination">
                        ...
                        <div className="pg-pagination__right">
                            ...
                            <span className="pg-pagination__text">{page} of {totalPages} pages ({displayRows.length} rows)</span>
                        </div>
                    </div>
                </div>
            </div>
